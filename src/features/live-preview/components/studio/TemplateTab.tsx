...
const NAV_STYLES: { id: T1NavStyle; label: string; desc: string }[] = [
    { id: "default", label: "Classic", desc: "Top dot indicator" },
    { id: "pill", label: "Pill", desc: "Rounded pill highlight" },
    { id: "floating", label: "Floating", desc: "Detached rounded bar" },
    { id: "glass", label: "Glass", desc: "Frosted glass effect" },
    { id: "minimal", label: "Minimal", desc: "Icons only, no bg" },
    { id: "dots", label: "Dots", desc: "Bottom dot indicator" },
    { id: "block", label: "Block", desc: "Full-width segments" },
    { id: "tab", label: "Tab", desc: "Top border indicator" },
];
        ...
        >
            ...
            <div>
                <div className="flex items-center gap-2 mb-2.5">
                    ...
                    <span className="text-xs font-bold text-foreground">Navigation Bar</span>
                </div>
                <div className="grid grid-cols-2 gap-1.5">
                    {NAV_STYLES.map(ns => {
                        const active = navStyle === ns.id;
                        return (
                            <button
                                key={ns.id}
                                onClick={() => setNavStyle(ns.id)}
                                className={`flex flex-col items-start p-2 rounded-lg border transition-all text-left ${active ? "border-transparent" : "border-slate-200 hover:border-slate-300"}`}
                                style={active ? { outline: `2px solid ${accent}`, outlineOffset: '-1px', borderColor: accent } : {}}
                            >
                                <span className={`text-[10px] font-bold ${active ? "" : "text-foreground"}`} style={active ? { color: accent } : {}}>
                                    {ns.label}
                                </span>
                                <span className="text-[8px] text-muted-foreground">{ns.desc}</span>
                            </button>
                        );
                    })}
                </div>
